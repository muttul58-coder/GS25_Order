...
하는 일:
    ...
    2. 카탈로그 JSON을 내려받아 상품명/가격을 가져옴
    3. 두 소스의 상품명을 교차 검증 (불일치하면 경고)
    4. products.js 생성
    5. BarcodeImgs/ 를 비우고 코드별 바코드 이미지를 다시 렌더링
...
def write_products_js(catalog, season, source_desc):
    entries, market = [], []
    for r in sorted(catalog, key=lambda r: code_key(r["code"])):
        code, name, price = r["code"], r["name"], r["price"]
        if isinstance(price, int):
            entries.append('  "%s": { "name": %s, "price": %d }'
                           % (code, json.dumps(name, ensure_ascii=False), price))
        else:
            # "시세반영" 등 숫자가 아닌 가격 → 단가를 직접 입력받는다
            market.append(code)
            entries.append('  "%s": { "name": %s, "price": 0, "marketPrice": true }'
                           % (code, json.dumps(name, ensure_ascii=False)))
...
def main():
    ...
    print("4) products.js 생성")
    market = write_products_js(catalog, args.season or "시즌", args.catalog)
