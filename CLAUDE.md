...
**Catalog deep link.** Clicking a barcode opens that product in the GS25 catalog
(`catalogSearchLink()` in `js/product.js`). The address comes from
`PROMO_CONFIG.catalogSearch`, generated by `catalog_search_url()` from the season's
카탈로그 주소 — never hardcode it, or next season's form opens last season's catalog
and shows a different product than the barcode scans as. The catalog is a React SPA
