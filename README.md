...
| **행사 지급수량 자동 계산** | 1+1, 2+1 등 행사 선택 시 덤을 포함한 지급수량 자동 산출 |
...
| **배송수량 검증** | 지급수량 대비 배송 수량 실시간 비교 및 경고 |
...
| `products.js` | 상품코드-상품명-가격 매핑. **자동 생성** |
...
```bash
python tools/update_season.py BarcodeSource/20260815.pdf --catalog https://gs25mobile.com/2026_2nd/products.json --season "2026 추석"
```

스크립트가 PDF와 카탈로그의 상품명을 교차 검증하고, 상품 수와 바코드 수가
1:1로 맞지 않으면 실패합니다. `[!]` 표시가 나오면 배포 전에 원인을 확인하세요.

필요 패키지: `pip install pdfplumber pymupdf`
