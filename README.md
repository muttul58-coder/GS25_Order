...
| **주류 배송 불가 안내** | 주류(양주/와인·소주/전통주 85개)는 택배 배송 불가 — 배송 상품 목록에서 제외하고 매장 수령 안내 표시, 전부 주류면 배송 정보 대신 안내로 대치 |
...
`배송불가` 는 주문에 주류가 있을 때만 실립니다(없으면 키 자체가 없습니다).
주문한 상품이 **전부 주류**면 받을 배송지가 없으므로 `주문목록` 은 빈 배열이 됩니다.
